            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div class="mb-3">
                        ...
                        <img id="gambar-preview" src="{{ asset('images/no-image.png') }}" alt="Preview Gambar" class="img-thumbnail mb-2" style="width: 100%; height: 200px; object-fit: cover;">
                        <input type="file" class="form-control @error('gambar') is-invalid @enderror" id="gambar" name="gambar" onchange="previewImage()">
                        <small class="text-muted">Ukuran maks: 2MB. Format: JPG, PNG, WEBP.</small>
                        @error('gambar')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('kategori') is-invalid @enderror" id="kategori" name="kategori" value="{{ old('kategori') }}" list="kategori-list" placeholder="Mis: Pemerintahan">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal" name="tanggal" value="{{ old('tanggal', now()->format('Y-m-d')) }}">
                        @error('tanggal')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
<script>
    ClassicEditor
        .create(document.querySelector('#konten'), {
            // Konfigurasi toolbar (opsional, tapi disarankan)
            toolbar: {
                items: [
                    'undo', 'redo',
                    '|', 'heading',
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.previewImage = function() {
            ...
            // (Kita biarkan logika preview yang lama)
        }
    });
</script>
@endpush